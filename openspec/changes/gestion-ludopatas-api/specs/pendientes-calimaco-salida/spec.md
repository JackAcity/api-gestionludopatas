...
### Requirement: Búsqueda de pendientes CALIMACO salida
El sistema SHALL exponer `POST /api/v1/pendientes/calimaco/salidas/busqueda`, que recibe `{corteIdActual: integer, maxReintentosPorSistema: integer, esReintentoForzado: boolean = false}` y ejecuta `dbo.SP_Pendientes_CALIMACO_Salida` (solo lectura, sin `ORDER BY`, sin reserva de filas). La respuesta SHALL ser un arreglo de 0..N objetos `{id, numeroDocumento, tipoDocumento, corteId, ultimoCorteId, calimacoUltimoCorteId, calimacoReintentos}`. Un arreglo vacío SHALL considerarse una respuesta `200` válida, no un error.
