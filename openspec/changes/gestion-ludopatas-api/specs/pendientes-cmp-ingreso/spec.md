...
### Requirement: Búsqueda de pendientes CMP ingreso
El sistema SHALL exponer `POST /api/v1/pendientes/cmp/ingresos/busqueda`, que recibe `{corteIdActual: integer, maxReintentosPorSistema: integer, esReintentoForzado: boolean = false}` y ejecuta `dbo.SP_Pendientes_CMP_Ingreso` (solo lectura, sin `ORDER BY`, sin reserva de filas). La respuesta SHALL ser un arreglo de 0..N objetos `{id, numeroDocumento, tipoDocumento, corteId, ultimoCorteId, cmpUltimoCorteId, cmpReintentos}`. Un arreglo vacío SHALL considerarse una respuesta `200` válida, no un error.
