...
### Requirement: Búsqueda de pendientes CALIMACO ingreso
El sistema SHALL exponer `POST /api/v1/pendientes/calimaco/ingresos/busqueda`, que recibe `{corteIdActual: integer, maxReintentosPorSistema: integer, esReintentoForzado: boolean = false}` y ejecuta `dbo.SP_Pendientes_CALIMACO_Ingreso` (solo lectura, sin `ORDER BY`, sin reserva de filas). La respuesta SHALL ser un arreglo de 0..N objetos `{id, numeroDocumento, tipoDocumento, corteId, ultimoCorteId, calimacoUltimoCorteId, calimacoReintentos}`. Un arreglo vacío SHALL considerarse una respuesta `200` válida, no un error.
